...
describe("AreCausallyDependent benchmark", async () => {
    ...
    const tests = [];
    const obj1 = new TopologyObject("peer1", new AddWinsSet());
    const obj2 = new TopologyObject("peer2", new AddWinsSet());
    const obj3 = new TopologyObject("peer3", new AddWinsSet());
    const cro1 = obj1.cro;
    const cro2 = obj2.cro;
    const cro3 = obj3.cro;
    cro1.add(1);
    ...
    bench("Causality check using BFS", async () => {
        for (let i = 0; i < samples; i++) {
            const result = obj1.hashGraph.areCausallyRelatedUsingBFS(tests[i][0], tests[i][1]);
        }
    });
    bench("Causality check using Bitsets", async () => {
        for (let i = 0; i < samples; i++) {
            const result = obj1.hashGraph.areCausallyRelatedUsingBitsets(tests[i][0], tests[i][1]);
        }
    });
});
