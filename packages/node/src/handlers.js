...
*/
export async function topologyMessagesHandler(node, stream, data) {
    let message;
    if (stream) {
        ...
        console.error("topology::node::messageHandler", "Stream and data are undefined");
        return;
    }
    switch (message.type) {
        ...
        case NetworkPb.Message_MessageType.SYNC:
            ...
            syncHandler(node, stream.protocol ?? "/topology/message/0.0.1", message.sender, message.data);
            break;
        case NetworkPb.Message_MessageType.SYNC_ACCEPT:
            ...
            syncAcceptHandler(node, stream.protocol ?? "/topology/message/0.0.1", message.sender, message.data);
            break;
...
*/
function updateHandler(node, data) {
    ...
    object.merge(updateMessage.vertices.map((v) => {
        return {
            hash: v.hash,
            nodeId: v.nodeId,
            operation: {
...
*/
function syncHandler(node, protocol, sender, data) {
    ...
    const requested = new Set(object.vertices);
    const requesting = [];
    ...
    if (requested.size === 0 && requesting.length === 0)
        return;
    const message = NetworkPb.Message.create({
        sender: node.networkNode.peerId,
        type: NetworkPb.Message_MessageType.SYNC_ACCEPT,
        // add data here
        data: NetworkPb.SyncAccept.encode(NetworkPb.SyncAccept.create({
            objectId: object.id,
            requested: [...requested],
            requesting,
        })).finish(),
...
/*
  ...
  operations array contain the full remote operations array
*/
function syncAcceptHandler(node, protocol, sender, data) {
    ...
    const vertices = syncAcceptMessage.requested.map((v) => {
        return {
            hash: v.hash,
            nodeId: v.nodeId,
            operation: {
...
/* data: { id: string } */
function syncRejectHandler(node, data) {
    // TODO: handle reject. Possible actions:
    // - Retry sync
    // - Ask sync from another peer
    // - Do nothing
}
export function topologyObjectChangesHandler(node, obj, originFn, vertices) {
    switch (originFn) {
        case "merge":
        ...
        case "callFn": {
            ...
            const message = NetworkPb.Message.create({
                ...
                data: NetworkPb.Update.encode(NetworkPb.Update.create({
                    objectId: obj.id,
                    vertices: vertices,
                })).finish(),
            });
