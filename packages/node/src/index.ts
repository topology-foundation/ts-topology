...
import type { EventHandler, StreamHandler } from "@libp2p/interface";
import {
	TopologyNetworkNode,
...
import { TopologyObjectStore, type TopologyObjectStoreCallback } from "./store";
...
export class TopologyNode {
	...
	async start(): Promise<void> {
		...
		this.networkNode.addMessageHandler(
			...
			async ({ stream }) => {
				const input = await streamToString(stream);
				if (!input) return;
	...
	async subscribeObject(
		objectId: string,
		fetch = false,
		peerId = "",
		subscribionCallback?: TopologyObjectStoreCallback,
	) {
		this.networkNode.subscribe(objectId);
