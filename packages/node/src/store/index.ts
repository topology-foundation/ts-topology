import type { ObjectPb } from "@topology-foundation/object";

export type TopologyObjectStoreCallback = (
	objectId: string,
	object: ObjectPb.TopologyObjectBase,
) => void;

export class TopologyObjectStore {
	// TODO: should be abstracted in handling multiple types of storage
	private _store: Map<string, ObjectPb.TopologyObjectBase>;
	private _subscriptions: Map<string, TopologyObjectStoreCallback[]>;

	constructor() {
		this._store = new Map<string, ObjectPb.TopologyObjectBase>();
		this._subscriptions = new Map<string, TopologyObjectStoreCallback[]>();
	}

	get(objectId: string): ObjectPb.TopologyObjectBase | undefined {
		return this._store.get(objectId);
	}

	put(objectId: string, object: ObjectPb.TopologyObjectBase) {
