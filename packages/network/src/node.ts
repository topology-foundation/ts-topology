...
import { rootNetwork } from "./configs/LogConfig";

const logStart = rootNetwork.getChildCategory("start");
const logSubscribe = rootNetwork.getChildCategory("subscribe");
const logUnsubscribe = rootNetwork.getChildCategory("unsubscribe");
const logBroadcastMessage = rootNetwork.getChildCategory("broadcastMessage");
const logSendMessage = rootNetwork.getChildCategory("sendMessage");
const logSendGroupMessageRandomPeer = rootNetwork.getChildCategory("sendMessageRandomTopicPeer");
...
export class TopologyNetworkNode {
  ...
  async start() {
    ...
    logStart.debug(() => `Successfuly started topology network w/ peer_id ${this.peerId}`);

    // TODO remove this or add better logger
    // we need to keep it now for debugging
    this._node.addEventListener("peer:connect", (e) =>
      logStart.debug(() => `peer:connect ${e.detail}`)
    );
    this._node.addEventListener("peer:discovery", (e) =>
      logStart.debug(() => `peer:discovery ${e.detail}`)
    );
    this._node.addEventListener("peer:identify", (e) =>
      logStart.debug(() => `peer:identify ${e.detail}`)
    );
  }

  subscribe(topic: string) {
    if (!this._node) {
      logSubscribe.error(() => `Node not initialized, please run .start()`);
      return;
    }
    ...
    try {
      ...
      logSubscribe.debug(() => `Successfuly subscribed the topic ${topic}`);
    } catch (e: any) {
      logSubscribe.error(() => e);
    }
  }

  unsubscribe(topic: string) {
    if (!this._node) {
      logUnsubscribe.error(() => `Node not initialized, please run .start()`);
      return;
    }

    try {
      this._pubsub?.unsubscribe(topic);
      logUnsubscribe.debug(() => `Successfuly unsubscribed the topic ${topic}`);
    } catch (e: any) {
      logUnsubscribe.error(() => e);
    }
  }
  ...
  async broadcastMessage(topic: string, message: Uint8Array) {
    try {
      ...
      logBroadcastMessage.debug(() => `Successfuly broadcasted message to topic ${topic}`);
    } catch (e: any) {
      logBroadcastMessage.error(() => e);
    }
  }
  ...
  async sendMessage(peerId: string, protocols: string[], message: string) {
    try {
      ...
      logSendMessage.debug(() => `Successfuly sent message to peer: ${peerId} with message: ${message}`);
    } catch (e: any) {
      logSendMessage.error(() => e);
    }
  }
  ...
  async sendGroupMessageRandomPeer(
    ...
    try {
      ...
      logSendGroupMessageRandomPeer.debug(() => `Successfuly sent message to peer: ${peerId} with message: ${message}`);
    } catch (e: any) {
      logSendGroupMessageRandomPeer.error(() => e);
    }
  }
