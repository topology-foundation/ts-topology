...
const grid_1 = require("./objects/grid");
...
const getColorForNodeId = (id) => {
    if (!colorMap.has(id)) {
        ...
        // Convert back to RGB
        [r, g, b] = (0, color_1.hslToRgb)(h, s, l);
        const color = (0, color_1.rgbToHex)(r, g, b); // Convert RGB to hex
...
async function moveUser(direction) {
    if (!gridCRO) {
        console.error("Grid CRO not initialized");
        alert("Please create or join a grid first");
        return;
...
async function main() {
    ...
    const button_create = (document.getElementById("createGrid"));
    button_create.addEventListener("click", async () => {
        topologyObject = await node.createObject(new grid_1.Grid());
        gridCRO = topologyObject.cro;
    ...
    button_connect.addEventListener("click", async () => {
        const croId = document.getElementById("gridInput")
            .value;
        try {
            topologyObject = await node.createObject(new grid_1.Grid(), croId, undefined, true);
            gridCRO = topologyObject.cro;
            createConnectHandlers();
            await addUser();
            render();
